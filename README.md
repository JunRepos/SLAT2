...
**교사** — 이메일 + 비밀번호로 로그인
- 대시보드: 팀 × 차시 현황표, 확인 대기·기한 지난 미제출 목록, 팀별 예산 잔액
- 기록 관리: 상태 변경(미제출 / 제출됨 / 작성·보완 요청 / 확인 완료), 피드백, 작성 담당 학생 지정, 선생님 직접 작성
- 출결: 차시별 출석·지각·조퇴·결석·공결 체크, 학생 기록의 참여자로 채우기, 학생별 출결 현황표
- 예산: 팀 한도 = 인원 × 25,000원, 구매 내역(구매일·품목·수량·단가·구매처·구매예정/완료) 기록, 남은 예산·초과 표시
...
| 경로 | 내용 | 권한 |
|---|---|---|
| `teams/{teamId}` | 계획서 (주제, 팀원, 차시, 예산), 구매 내역(`purchases`) | 교사 쓰기 · 팀원 읽기 |
| `teams/{teamId}/logs/{차시}` | 차시 기록, 교사 피드백·담당 학생 | 팀원 작성 · 교사 검토·작성 |
| `teams/{teamId}/attendance/{차시}` | 교사 출결 체크 | 교사 쓰기 · 팀원 읽기 |
